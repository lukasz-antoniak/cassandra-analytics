...
public final class SSTableTombstoneWriter implements Closeable
{
    ...
    private void rawAddRow(List<ByteBuffer> values) throws InvalidRequestException, IOException
    {
        ...
        List<ByteBuffer> keys = delete.buildPartitionKeyNames(options, ClientState.forInternalCalls());
        ...
        if (delete.hasSlices())
        {
            ...
            // TODO(lantoniak): Check correctness.
            Slices slices = delete.getRestrictions().getSlices(options);
        ...
        SortedSet<Clustering<?>> clusterings = delete.createClustering(options, ClientState.forInternalCalls());
        try
        {
            for (ByteBuffer key : keys)
            {
    ...
    public static class Builder
    {
        ...
        public SSTableTombstoneWriter build()
        {
            ...
            TableMetadata tableMetadata = CassandraSchema.apply(schema -> {
                ...
                {
                    schema.submit(SchemaTransformations.addKeyspace(SystemKeyspace.metadata(), false));
                }
                ...
                if (schema.getKeyspaceMetadata(keyspaceName) == null)
                {
                    KeyspaceMetadata ksm = KeyspaceMetadata.create(keyspaceName,
                                                                   ...
                                                                   Types.none(),
                                                                   UserFunctions.none());
                    schema.submit(SchemaTransformations.addKeyspace(ksm, false));
                }
                ...
                if (table == null)
                {
                    ...
                    schema.submit(new SchemaTransformation()
                    {
                        ...
                        {
                            KeyspaceMetadata km = ksm.withSwapped(ksm.tables.with(finalTable)).withSwapped(types);

                            // TODO(lantoniak): Remove?
//                            KeyspaceMetadata km = clusterMetadata.schema.getKeyspaceMetadata(keyspaceName);
//                            km = km.withSwapped(km.tables.withSwapped(finalTable)).withSwapped(types);
        ...
        private TableMetadata createTable(Types types)
        {
            ...
            TableMetadata.Builder builder = statement.builder(types, null);
            if (partitioner != null)
            {
