...
/**
 * Streams rows from a sequence of compaction scanners, opening one scanner for each bounded batch of partition keys.
 * The candidate iterator is globally ordered, so concatenating scanners preserves partition order while keeping
 * memory bounded by the configured batch size.
 */
public final class BatchedCompactionStreamScanner implements StreamScanner<RowData>
{
    ...
    public boolean next() throws IOException
    {
        ...
        {
            // Once a previous batch has emitted rows we cannot transparently switch back to a full scan without
            // duplicating results. Release SAI/read resources and let Spark retry the task instead.
            close();
            throw exception;
        }
    }
    ...
    private static void closeQuietly(@Nullable Closeable closeable)
    {
        ...
        {
            // Close is best effort. Any read error has already been surfaced by next().
        }
    }
}
