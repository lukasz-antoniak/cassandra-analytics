...
import static org.apache.cassandra.serializers.CollectionSerializer.sizeOfCollectionSize;
import static org.apache.cassandra.serializers.CollectionSerializer.sizeOfValue;
import static org.apache.cassandra.serializers.CollectionSerializer.writeValue;
...
public abstract class ComplexTypeBuffer extends AbstractComplexTypeBuffer
{
    ...
    public ByteBuffer pack()
    {
        ...
        for (ByteBuffer value : buffers)
            size += sizeOfValue(value, ByteBufferAccessor.instance);

        ByteBuffer result = ByteBuffer.allocate(sizeOfCollectionSize() + size);
        result.putInt(elements()); // write collection size
        for (ByteBuffer value : buffers)
        {
            writeValue(result, value, ByteBufferAccessor.instance);
        }
        return ByteBufferAccessor.instance.valueOf(result.flip());
    }
}
